...
sphinx_docx supports third party extensions which generate well-formed `docutils document tree`_

However, there are some extensions to generate the tree with their original docutils nodes.
Because sphinx_docx does not support the nodes, the all nodes are ignored when building the document.
...
.. code-block:: python
   ...
   def setup(app):
       ...
       def docx_visit_plantuml(self, node):
           ...
           # sphinx_docx provides useful methods. See sphinx_docx API reference.
           self.visit_image_node(node, alt, get_filepath)
       # Add the visit method to sphinx_docx
       import sphinx_docx
